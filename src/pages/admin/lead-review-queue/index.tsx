import type { LeadReviewQueueItem } from "@/api/lead-workflows";
import {
  useApproveLeadTask,
  useLeadReviewQueue,
  useRejectLeadTask,
...
export function LeadReviewQueuePage() {
  ...
  const { data: response, isLoading } = useLeadReviewQueue(
    statusParam,
    currentPage,
    pageLimit,
  );
  ...
  const counts = {
    in_review: queue.filter((i) => i.status === "in_review").length,
      ...
      {isLoading ? (
        ...
        <>
          <VStack gap={3} align="stretch">
            {queue.map((item) => (
              <LeadReviewCard key={item.id} item={item} />
            ))}
          </VStack>
...
function LeadReviewCard({ item }: { item: LeadReviewQueueItem }) {
  const queryClient = useQueryClient();
  const [approveOpen, setApproveOpen] = useState(false);
  const [rejectOpen, setRejectOpen] = useState(false);

  const approveMutation = useApproveLeadTask(item.leadId);
  const rejectMutation = useRejectLeadTask(item.leadId);
  ...
  const handleApprove = (notes: string) => {
    approveMutation.mutate(
      { taskId: item.id, notes: notes || undefined },
      {
        onSettled: () => {
          setApproveOpen(false);
          queryClient.invalidateQueries({
            queryKey: leadWorkflowKeys.reviewQueue(),
          });
        },
      },
    );
        ...
        <Box flex={1}>
          ...
          <Flex gap={2} mt={0.5} flexWrap="wrap" align="center">
            ...
            <Text fontSize="11px" color="fg.subtle">
              {item.leadName} · {stageLabels[item.pipelineStage] ?? item.pipelineStage}
            </Text>
          </Flex>
